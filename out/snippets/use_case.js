...
exports.generateFakeRepository = generateFakeRepository;
function generateFakeRepository(widgetName, classNameWithoutMaj) {
    return `import 'package:yotta_city/data/dto/${classNameWithoutMaj}_dto.dart';
import 'package:yotta_city/domain/mapper/${classNameWithoutMaj}_mapper.dart';
import 'package:yotta_city/domain/${classNameWithoutMaj}/entity/${classNameWithoutMaj}.dart';
...
class ${widgetName}Repository {
  ...
  static final ${widgetName}Repository _singleton =
      ${widgetName}Repository._internal();
  ...
  ${widgetName} add${widgetName}(${widgetName} ${classNameWithoutMaj}) {
    _count++;
    _fakeDatabase.add(
      ${widgetName}Mapper().convert(
        ${classNameWithoutMaj}.copyWith(
          id: _uniqueId,
        ),
      ),
    );
  ...
  bool update${widgetName}(${widgetName} ${classNameWithoutMaj}) {
    ...
    if (is${widgetName}InDatabase()) {
      final ${widgetName}Dto updated${widgetName} = ${widgetName}Mapper().convert(
        ${classNameWithoutMaj},
      );
  ...
  bool remove${widgetName}(${widgetName} ${classNameWithoutMaj}) {
    if (_fakeDatabase.any(
      (${classNameWithoutMaj}Dto) => ${classNameWithoutMaj}Dto.id == ${classNameWithoutMaj}.id,
    )) {
      return update${widgetName}(${classNameWithoutMaj}.copyWith(isDeleted: true));
    } else {
      return false;
    }
  }

  bool undoRemove${widgetName}(${widgetName} ${classNameWithoutMaj}) {
    if (_fakeDatabase.any(
      (${classNameWithoutMaj}Dto) => ${classNameWithoutMaj}Dto.id == ${classNameWithoutMaj}.id,
    )) {
      return update${widgetName}(${classNameWithoutMaj}.copyWith(isDeleted: false));
    } else {
      return false;
    }
  }
    ...
    if (_fakeDatabase.any((${classNameWithoutMaj}) => ${classNameWithoutMaj}.id == ${classNameWithoutMaj}Id)) {
      return ${widgetName}Mapper().convert(
        _fakeDatabase.firstWhere(
          (${classNameWithoutMaj}) =>
              ${classNameWithoutMaj}.id == ${classNameWithoutMaj}Id && ${classNameWithoutMaj}.isDeleted == false,
        ),
      );
    }
    return null;
  ...
  List<${widgetName}> _get${widgetName}ListWhere({
    ...
    var filteredResult =
        _fakeDatabase
            .map(
              (dto) =>
                  ${widgetName}Mapper().convert<${widgetName}Dto, ${widgetName}>(dto),
            )
            .where((${classNameWithoutMaj}) => ${classNameWithoutMaj}.isDeleted == false)
            .where(filter)
            .toList();
  ...
  List<${widgetName}> get${widgetName}List({
    ...
    return _get${widgetName}ListWhere(
      ...
      filter: filter ?? (${classNameWithoutMaj}) => ${classNameWithoutMaj}.isDeleted == false,
    );
  }
}`;
}
